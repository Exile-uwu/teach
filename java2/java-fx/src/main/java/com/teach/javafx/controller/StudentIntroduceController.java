...
public class StudentIntroduceController extends ToolController {
    ...
    private ObservableList<Map> observableList2= FXCollections.observableArrayList();
    ...
    @FXML
    private PieChart pieChart2;   //成绩分布饼图控件
    ...
    public void getIntroduceData(){
        ...
        GPA.setText(CommonMethod.getString(info,"GPA"));
        ...
        List<Map>innovationList = (List)data.get("innovationList");
        ...
        for (Map m: assessList) {
            observableList2.addAll(FXCollections.observableArrayList(m));
        }
        assessTable.setItems(observableList2);  // 成绩表数据显示
        ...
        for(Map m:innovationList) {
            chartData2.add(new PieChart.Data(m.get("title").toString(),Double.parseDouble(m.get("value").toString())));
        }
        pieChart2.setData(chartData2);  //成绩分类表显

        XYChart.Series<String, Number> seriesFee = new XYChart.Series<>();
        seriesFee.setName("日常消费");
        for(Map m:feeList)
            seriesFee.getData().add(new XYChart.Data<>(m.get("day").toString(),Double.parseDouble(m.get("money").toString())));
    ...
    @FXML
    protected void onGPAButtonClick() {
        DataRequest req = new DataRequest();
        req.put("studentId", studentId);
        DataResponse res = HttpRequestUtil.request("/api/score/GPA",req);
        if(res.getCode() == 0) {
            MessageDialog.showDialog("计算成功！");
        }
        else {
