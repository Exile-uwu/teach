...
public class EditController extends ToolController{
    @FXML
    ...
    public void initialize() {
        ...
        req.put("numName","");
        res = HttpRequestUtil.request("/api/innovation/getSingleInnovationList",req); //从后台获取所有学生信息列表集合
    ...
    protected void changeInnovationInfo() {
        ...
        DataResponse res = HttpRequestUtil.request("/api/innovation/getSingleInnovationInfo",req);
        if(res.getCode() != 0){
    ...
    protected void onQueryButtonClick() {
        ...
        DataResponse res = HttpRequestUtil.request("/api/innovation/getSingleInnovationList2",req);
        if(res != null && res.getCode()== 0) {
            innovationList = (ArrayList<Map>)res.getData();
            setTableViewData();
        }
    ...
    protected void onDeleteButtonClick() {
        ...
        DataRequest req = new DataRequest();
        req.put("innovationId", innovationId);
        DataResponse res = HttpRequestUtil.request("/api/innovation/innovationSingleDelete",req);
        if(res.getCode() == 0) {
    ...
    protected void onSaveButtonClick() {
        ...
        if(contentField.getText().length()==0){
            MessageDialog.alertDialog("请填写项目内容！");
            return;
        }
        ...
        req.put("innovationId", innovationId);
        req.put("form", form);
        DataResponse res = HttpRequestUtil.request("/api/innovation/innovationSingleEditSave",req);
        if(res.getCode() == 0) {
