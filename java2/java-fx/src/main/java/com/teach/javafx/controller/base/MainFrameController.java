...
public class MainFrameController {
     ...
     * HttpRequestUtil.request("/api/base/getMenuList",new
     * DataRequest())加载用菜单管理功能，维护的菜单
    ...
    public void initialize() {
        ...
        menu = new Menu("教育管理系统");

        if (role.equals("ROLE_ADMIN")) {
            item = new MenuItem();
            item.setText("管理员端介绍");
            item.setOnAction(e -> changeContent("base/admin-home-panel", "管理员端介绍"));
            menu.getItems().add(item);
            changeContent("base/admin-home-panel", "管理员端介绍");
        }
        if (role.equals("ROLE_STUDENT")) {
            item = new MenuItem();
            item.setText("学生端介绍");
            item.setOnAction(e -> changeContent("base/student-home-panel", "学生端介绍"));
            menu.getItems().add(item);
            changeContent("base/student-home-panel", "学生端介绍");
        }
        if (role.equals("ROLE_TEACHER")) {
            item = new MenuItem();
            item.setText("教师端介绍");
            item.setOnAction(e -> changeContent("base/teacher-home-panel", "教师端介绍"));
            menu.getItems().add(item);
            changeContent("base/teacher-home-panel", "教师端介绍");
        }
        ...
        if (role.equals("ROLE_STUDENT") || role.equals("ROLE_TEACHER")) {
            ...
            if (role.equals("ROLE_STUDENT")) {
                item = new MenuItem();
                item.setText("家庭信息");
                item.setOnAction(e -> changeContent("relationStudent-panel", "家庭信息"));
                menu.getItems().add(item);
                item = new MenuItem();
                item.setText("请假信息");
                item.setOnAction(e -> changeContent("leaveStudent-panel", "请假信息"));
                menu.getItems().add(item);
                item = new MenuItem();
                item.setText("消费信息");
                item.setOnAction(e -> changeContent("feeStudent-panel", "消费信息"));
                menu.getItems().add(item);
            }
            menuBar.getMenus().add(menu);
        }
    ...
    protected void onLogoutMenuClick(ActionEvent event) {
        ...
        try {
            ...
            MainApplication.resetStage2("登录", scene);
        } catch (IOException e) {
    ...
    /**
     * ToolController getCurrentToolController() 获取当前显示的面板的控制对象，
     * 如果面板响应编辑菜单中的编辑命名，交互控制需要继承 ToolController， 重写里面的方法
     * 
     * @return
     */
