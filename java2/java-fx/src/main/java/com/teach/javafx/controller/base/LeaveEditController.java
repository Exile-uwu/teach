...
public class LeaveEditController extends ToolController{
    @FXML
    ...
    public void initialize() {
        ...
        req.put("numName","");
        res = HttpRequestUtil.request("/api/leave/getSingleLeaveList",req); //从后台获取所有学生信息列表集合
    ...
    protected void changeLeaveInfo() {
        ...
        DataResponse res = HttpRequestUtil.request("/api/leave/getSingleLeaveInfo",req);
        if(res.getCode() != 0){
    ...
    protected void onQueryButtonClick() {
        ...
        DataResponse res = HttpRequestUtil.request("/api/leave/getSingleLeaveList2",req);
        if(res != null && res.getCode()== 0) {
            leaveList = (ArrayList<Map>)res.getData();
            setTableViewData();
        }
    ...
    protected void onDeleteButtonClick() {
        ...
        DataRequest req = new DataRequest();
        req.put("leaveId", leaveId);
        DataResponse res = HttpRequestUtil.request("/api/leave/leaveSingleDelete",req);
        if(res.getCode() == 0) {
    ...
    protected void onSaveButtonClick() {
        ...
        if(reasonField.getText().length()==0){
            MessageDialog.alertDialog("请填写请假原因！");
            return;
        }
        ...
        req.put("leaveId", leaveId);
        req.put("form", form);
        DataResponse res = HttpRequestUtil.request("/api/leave/leaveSingleEditSave",req);
        if(res.getCode() == 0) {
