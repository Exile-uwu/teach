...
public class StudentController {
    ...
    @Autowired
    private InnovationRepository innovationRepository;  //人员数据操作自动注入
    @Autowired
    private DictionaryInfoRepository dictionaryInfoRepository;  //人员数据操作自动注入
    @Autowired
    private StudentRepository studentRepository;  //学生数据操作自动注入
    @Autowired
    private AssessRepository assessRepository;  //学生数据操作自动注入
    @Autowired
    ...
    public synchronized Integer getNewDictonaryId(){  //synchronized 同步方法
    ...
    public DataResponse studentDelete(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if(s != null) {
            Optional<DictionaryInfo> dOp = dictionaryInfoRepository.findByLabel(s.getPerson().getName());
            if(dOp.isPresent()) {
                dictionaryInfoRepository.delete(dOp.get()); //删除对应该学生的账户
            }
    ...
    public DataResponse studentEditSave(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if(s == null) {
            ...
            DictionaryId = getNewDictonaryId();//插入新的Student记录
            d = new DictionaryInfo();
            d.setId(DictionaryId);
            d.setValue(s.getStudentId().toString());
            d.setLabel(p.getName());
            d.setPid(22);
            dictionaryInfoRepository.saveAndFlush(d);
    ...
    public List getStudentAssessList(String acceptorId){
        List<Assess> sList =assessRepository.findAssessListByAcceptor(acceptorId);
        List list = new ArrayList();
        if(sList == null || sList.size() == 0)
            return list;
        Map m;
        ...
        for(Assess s:sList){
            ...
            Optional<Student>st=studentRepository.findByStudentId(s.getStudentId());
            m.put("assessorName",st.get().getPerson().getName());
    ...
    public List getStudentInnovationList(List<Innovation> sList){
        String title[]={"社会实践","学科竞赛","科技成果","培训讲座","创新项目","课外实习"};
        int count[]= new int[7];
        List list = new ArrayList();
        if(sList == null || sList.size() == 0)
            return list;
        ...
        for(Innovation s:sList){
            ...
            if(Integer.parseInt(s.getType())== 2)
            {
                count[1]++;
                continue;
            }
            if(Integer.parseInt(s.getType())== 3)
            {
                count[2]++;
                continue;
            }
            if(Integer.parseInt(s.getType())== 4)
            {
                count[3]++;
                continue;
            }
            if(Integer.parseInt(s.getType())== 5)
            {
                count[4]++;
                continue;
            }
            if(Integer.parseInt(s.getType())== 6)
            {
                count[5]++;
                continue;
            }
    ...
    public DataResponse getStudentIntroduceData(@Valid @RequestBody DataRequest dataRequest) {
        ...
        List<Innovation> iList = innovationRepository.findInnovationListByStudentId(s.getStudentId());
        Map data = new HashMap();
        data.put("info",info);
        data.put("scoreList",getStudentScoreList(sList));
        data.put("assessList",getStudentAssessList(String.valueOf(s.getStudentId())));
        data.put("markList",getStudentMarkList(sList));
        data.put("innovationList",getStudentInnovationList(iList));
