...
public interface FeeRepository extends JpaRepository<Fee,Integer> {
    ...
    @Query(value= "from Fee f where f.studentId = ?1 and f.remark like %?2% order by day")
    List<Fee> findFeeListByStudentIdAndRemark(Integer studentId,String numName);

    @Query(value = "from Fee where ?1='' or name like %?1%")
