...
public class ActivityController {
    ...
    @Autowired
    private ActivityRepository activityRepository;  //学生数据操作自动注入
    @Autowired
    ...
    public Map getMapFromActivity(Activity s) {
        ...
        m.put("type",type);
        m.put("typeName", ComDataUtil.getInstance().getDictionaryLabelByValue("RCHD", type)); //性别类型的值转换成数据类型名
    ...
    public List getSingleActivityMapList(Integer studentId) {
        List dataList = new ArrayList();
    ...
    public List getSingleActivityMapList2(Integer studentId,String numName) {
        List dataList = new ArrayList();
        List<Activity> sList = activityRepository.findActivityListByStudentIdAndName(studentId,numName);  //数据库查询操作
        if(sList == null || sList.size() == 0)
            return dataList;
        for(int i = 0; i < sList.size();i++) {
            dataList.add(getMapFromActivity(sList.get(i)));
    ...
    public OptionItemList getActivityItemOptionList(@Valid @RequestBody DataRequest dataRequest) {
        List<Activity> sList = activityRepository.findActivityListByNumName("");  //数据库查询操作
        OptionItem item;
        List<OptionItem> itemList = new ArrayList();
        for (Activity s : sList) {
            itemList.add(new OptionItem(s.getActivityId(), s.getName(), s.getName()+"-"+s.getType()));//此处可能有问题-hzd
        }
    ...
    @PostMapping("/getSingleActivityList")
    @PreAuthorize("hasRole('STUDENT')")
    public DataResponse getSingleActivityList(@Valid @RequestBody DataRequest dataRequest) {
        ...
        List dataList = getSingleActivityMapList(st.getStudentId());
        return CommonMethod.getReturnData(dataList);  //按照测试框架规范会送Map的list
    }

    @PostMapping("/getSingleActivityList2")
    @PreAuthorize("hasRole('STUDENT')")
    public DataResponse getSingleActivityList2(@Valid @RequestBody DataRequest dataRequest) {
        String numName= dataRequest.getString("numName");
        Integer userId = CommonMethod.getUserId();
        Optional<User> uOp = userRepository.findByUserId(userId);  // 查询获得 user对象
        if(!uOp.isPresent())
        ...
        List dataList = getSingleActivityMapList2(st.getStudentId(),numName);
        return CommonMethod.getReturnData(dataList);  //按照测试框架规范会送Map的list
    }
    ...
    public DataResponse activityDelete(@Valid @RequestBody DataRequest dataRequest) {
        Integer activityId = dataRequest.getInteger("activityId");  //获取student_id值
        Activity s= null;
        ...
        if(s != null) {
            activityRepository.delete(s);    //首先数据库永久删除学生信息
            // 然后数据库永久删除学生信息
        }
        return CommonMethod.getReturnMessageOK();  //通知前端操作正常
    }

    @PostMapping("/activitySingleDelete")
    @PreAuthorize(" hasRole('STUDENT')")
    public DataResponse activitySingleDelete(@Valid @RequestBody DataRequest dataRequest) {
        Integer activityId = dataRequest.getInteger("activityId");  //获取student_id值
        Activity s= null;
        Optional<Activity> op;
        if(activityId != null) {
    ...
    public DataResponse getActivityInfo(@Valid @RequestBody DataRequest dataRequest) {
        ...
        return CommonMethod.getReturnData(getMapFromActivity(s)); //这里回传包含学生信息的Map对象
    }

    @PostMapping("/getSingleActivityInfo")
    @PreAuthorize("hasRole('STUDENT')")
    public DataResponse getSingleActivityInfo(@Valid @RequestBody DataRequest dataRequest) {
        Integer activityId = dataRequest.getInteger("activityId");
        Activity s= null;
        Optional<Activity> op;
        if(activityId != null) {
    ...
    public DataResponse activityEditSave(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if(s == null) {
            s = new Activity();   // 创建实体对象
            activityId=getNewActivityId();
            s.setActivityId(activityId);

            activityRepository.saveAndFlush(s);  //插入新的Student记录
        }
        ...
        activityRepository.save(s);  //修改保存学生信息
        return CommonMethod.getReturnData(s.getActivityId());  // 将studentId返回前端
    }

    @PostMapping("/activitySingleEditSave")
    @PreAuthorize(" hasRole('STUDENT')")
    public DataResponse activitySingleEditSave(@Valid @RequestBody DataRequest dataRequest) {
        Integer activityId = dataRequest.getInteger("activityId");
        Map form = dataRequest.getMap("form"); //参数获取Map对象
        String num = CommonMethod.getString(form,"num");  //Map 获取属性的值
        Activity s= null;
