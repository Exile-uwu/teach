...
public class BaseController {
    ...
    @Autowired
    private StudentRepository studentRepository;  //用户数据操作自动注入
    @Autowired
    private TeacherRepository teacherRepository;  //用户数据操作自动注入
    @Autowired
    ...
    public DataResponse getUserList(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if(u.getUserType().getId()==2) {
            ...
            Student st= sOp.get();
            return CommonMethod.getReturnData(st.getPerson().getName());  //按照测试框架规范会送Map的list
        }  // 查询获得 Student对象
        if(u.getUserType().getId()==3) {
            Optional<Teacher> tOp = teacherRepository.findByPersonPersonId(u.getUserId());
            if(!tOp.isPresent())
                return CommonMethod.getReturnMessageError("学生不存在！");
            Teacher t= tOp.get();

            return CommonMethod.getReturnData(t.getPerson().getName());  //按照测试框架规范会送Map的list
        }
        return CommonMethod.getReturnData(userId);
